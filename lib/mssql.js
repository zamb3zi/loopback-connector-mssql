...
function DateTranslator() {
    this.setOffset(moment().utcOffset());
    this.update();
}
DateTranslator.prototype.update = function() {
    var off = process.env.MSSQL_UTC_OFFSET;
    if(off !== this.UTC_OFFSET) {
        this.UTC_OFFSET = off;
        this.setOffset(parseInt(off, 10))
    }
};
DateTranslator.prototype.setOffset = function(off) {
    console.log('>>>>>>>>>>>>>>>>>>>>>>> SETTING mssql utc offset', off);
    this.utcOffset = off;
    // convert the offset in minutes into ISO 8601 format
    var fmt = '';
    if(!off)
        fmt = 'Z';
    else {
        if(off < 0) {
            ...
            fmt += '+';
        var mins = off % 60, hours = (off - mins) / 60;
        if(hours < 10) fmt += '0';
        fmt += hours;
        if(mins) {
            if(mins < 10) fmt += '0';
    ...
    console.log('>>>>>>>>>>>>>>>>>>>>>>> fmt =', fmt);
    this.formattedOffset = fmt;
};
DateTranslator.prototype.parse = function(s) {
    return moment(s).subtract(this.utcOffset, 'minutes').toDate();
    //if(/Z|(?:[\-+])(?:\d{2}):?(?:\d{2})$/.test(s))
    //    return moment(s).toDate();
    //return moment(s + this.formattedOffset).toDate();
};
DateTranslator.prototype.format = function(s) {
    return moment(s).utcOffset(this.utcOffset).format('Y-MM-DDTHH:mm:ss.SSS');
};
var dateTranslator = new DateTranslator;

function setMomentUTCOffset() {
    return setMomentUTCOffset.value;
}
...
function escape(val) {
  ...
  switch (typeof val) {
    ...
    case 'object':
      ...
      break;
    case 'string':
      if (val.slice(0, 7) === 'RAWHEX!')
        return '0x' + val.slice(7);
      break;
  }

  // single-quotes are the only characters that need escaping in TSQL
  // string literals.
