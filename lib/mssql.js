...
MsSQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  if (transaction) {
    if(transaction.connector === this) {
      const {connection} = transaction;
      if(connection) {
        const {_lock} = connection;
        if(_lock) {
          debug('Execute SQL in a transaction');
          _lock.acquire('', done => executeQuery(connection, done), innerCB);
          return;
        }
        debug('###### missing lock');
      }
      debug('###### missing connection');
    }
    debug('###### unknown transaction');
  }
