...
DateTranslator.prototype.update = function() {
    this.timezone = process.env.MSSQL_TIMEZONE || moment.tz.guess();
};
DateTranslator.prototype.parse = function(s) {
    // ### unless the datasource is configured with the tedious option
    //     useUTC = false in which case we need to use UTC ###
    const m = moment.tz(s, this.timezone);
    m.subtract(m.utcOffset(), 'minutes');
    return m.toDate();
};
DateTranslator.prototype.format = function(s) {
    const m =  moment.tz(s, this.timezone);
    // datetimes are rounded in tsql to the nearest 0, 3 or 7 milliseconds. if the
    // value's milliseconds ends in 9, we ensure that it is rounded down to 7
    // instead of up to 0, by subtracting one millisecond.
    // this ensures that values like endDate.endOf('day') stay on the same day rather
    // than being rounded up to the next day.
    // see: https://docs.microsoft.com/en-us/sql/t-sql/data-types/datetime-transact-sql
    const ms = m.milliseconds();
    if((ms % 10) === 9)
      m.milliseconds(ms - 1);
    // console.log('formatting', s, this.timezone, m.format('Y-MM-DDTHH:mm:ss.SSS'));
    return m.format('Y-MM-DDTHH:mm:ss.SSS');
};
var dateTranslator = new DateTranslator;
...
MsSQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  var executeQuery = function(connection, cb) {
    ...
    if (Array.isArray(params) && params.length > 0) {
      for (var i = 0, n = params.length; i < n; i++) {
        if (typeof params[i] === 'number' &&
          ...
          } else if(params[i] === nullBuffer) {
            request.input('param' + (i + 1), mssql.VarBinary(1), null);
        } else {
  ...
  if (transaction && transaction.connector === this && transaction.connection && transaction.connection.queue) {
    debug('Execute SQL in a transaction');

    transaction.connection.queue(function(next) {
      executeQuery(transaction.connection, function(err, data) {
        innerCB(err, data);
        next();
      });
    });
    ...
    executeQuery(this.client, innerCB);
  }
};
...
MsSQL.prototype.toColumnValue = function(prop, val) {
  ...
  if (prop.type === Date || prop.type.name === 'Timestamp') {
    //console.log('toValue', val, '->', dateTranslator.format(val));
    val = dateTranslator.format(val);
    return val;
  }
...
MsSQL.prototype.fromColumnValue = function(prop, val) {
  ...
  if (type === Boolean)
    return !!val; //convert to a boolean type from number
  if (type === Date) {
    //console.log('fromValue', val, '->', dateTranslator.parse(val).toISOString());
    val = dateTranslator.parse(val);
  }
  return val;
};
